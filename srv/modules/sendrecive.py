...
from srv.modules.terminal import print_dbg, print_err, handle_terminal
from srv.modules.sockets import send, recive, TCP_PDU, config_TCP
...
def _log_element_change(pdu):
    try:
        ...
        f.write(s + "\n")
        print_dbg(f"Guardat a {pdu.txId}.data: {s}")
        f.close()
        return True
    except OSError as e:
        return False
...
class SendReciveService:
    ...
    def handle_outgoing_conexion(self, type, device, element, value=""):
        sock = config_TCP(0)
        try:
            print_dbg(f"Connect amb {device.ip} i port {device.portTCP}")
        ...
        out_pkt = TCP_PDU(type, self.cfg.id, device.commId, element, value, device.id)
        send(sock, out_pkt)
